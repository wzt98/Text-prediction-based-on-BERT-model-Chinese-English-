...
def read_corpus(file_path):
    ...
    labels = []
    seg = pkuseg.pkuseg()
    fout = pd.read_excel(file_path, usecols=[0, 1])
    # dataframe转化成list
    # print(fout)
...
train = read_corpus('./en-topic.xlsx')
print("after read")
...
total_targets = total_targets.view(-1, 1)  # 3940
...
model = BertForSequenceClassification.from_pretrained(model_name, num_labels=5)  # num_labels表示2个分类,好评和差评 标签分类总数
...
def read_test(file_path):
    src_data = []
    fout = pd.read_excel(file_path, usecols=[9])  # 预测第几列的数据就填几
    # dataframe转化成list
    fout = fout.values.tolist()
    # 去掉第一行
    fout = fout[1:]
